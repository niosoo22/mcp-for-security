...
server.tool("do-alterx", "Execute Alterx, a tool that generates domain wordlists using pattern-based permutations for subdomain discovery and DNS enumeration", {
    domain: zod_1.z.string().describe("Target domain or subdomains to use as a base for creating permutations (accepts stdin input, comma-separated values, or file path)"),
    pattern: zod_1.z.string().describe(`Pattern template for generating wordlist variations. Multiple patterns can be specified using comma-separation.
            ...
            `),
    outputFilePath: zod_1.z.string().nullable().describe("Path where the generated wordlist should be saved (optional)")
    ...
    alterx = (0, child_process_1.spawn)(args[0], alterxArgs);
    let output = '';
    // Handle stdout
    alterx.stdout.on('data', (data) => {

...
server.tool("do-scoutsuite-aws", "Performs an AWS cloud security audit using Scout Suite for the given target settings, allowing service/region filtering and multiple authentication methods.", {
    ...
    if (max_workers)
        scoutSuiteArgs.push("--max-workers", max_workers.toString());
    if (services?.length) {
    ...
    if (profile)
        scoutSuiteArgs.push("--profile", profile);
    if (acces_keys)
        scoutSuiteArgs.push("--access-keys"); // This is a flag, presence indicates manual credentials
    if (access_key_id)
        scoutSuiteArgs.push("--access-key-id", access_key_id);
    if (secret_acces_key)
        scoutSuiteArgs.push("--secret-access-key", secret_acces_key);
    if (session_token)
        scoutSuiteArgs.push("--session-token", session_token);
    if (regions)
        scoutSuiteArgs.push("--regions", regions);
    if (exclude_regions)
        scoutSuiteArgs.push("--exclude-regions", exclude_regions);
    if (ip_ranges)
        scoutSuiteArgs.push("--ip-ranges", ip_ranges);
    if (ip_ranges_name_key)
        scoutSuiteArgs.push("--ip-ranges-name-key", ip_ranges_name_key);
    let output = "";
    ...
    scoutSuite.on('data', function (data) {
        output += data.toString();
    });
    // Handle process completion
    return new Promise((resolve, reject) => {
        scoutSuite.on('close', function (code) {
        ...
        scoutSuite.on('error', function (error) {
